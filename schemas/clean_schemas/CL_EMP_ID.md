...
**Source:** https://datahandbook.epic.com/ClarityDictionary/Details?tblName=CL_EMP_ID

## Description

The CL_EMP_ID table contains the system ID numbers for your users. Each record may have multiple IDs; therefore, a line number is used to identify each identification number for a record.
...
| --- | --- | --- |
| USER_NUMBER_ID | VARCHAR (18) | The unique ID number assigned to the user record. |
| LINE | No | This column holds the line number used to identify an ID for a user record. Since user records can have multiple IDs, a line number is used to identify an individual ID on that record. |
| CM_PHY_OWNER_ID | VARCHAR (25) | ID of the physical deployment owner for this record.  Physical owners will be where the data is hosted, either on the cross-over server or the owner deployment. |
| CM_LOG_OWNER_ID | VARCHAR (25) | ID of the logical deployment owner for this record.  Logical owners show the deployment where the record was created but doesn't represent if the record is a part of version skew. |
| MPI_ID_TYPE_ID | NUMERIC (18,0) | The MPI ID type linked to the record. |
