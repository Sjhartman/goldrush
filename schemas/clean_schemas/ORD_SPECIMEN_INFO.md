...
**Source:** https://datahandbook.epic.com/ClarityDictionary/Details?tblName=ORD_SPECIMEN_INFO
...
| SPECIMEN_LAB_ID | NUMERIC (18,0) | The lab (LLB) record ID responsible for filing the specimen information through the interface. |
| SPECIMEN_ACTIVE_C | INTEGER |  |
| SPECIMEN_ACTIVE_UTC_DTTM | DATETIME (UTC) | The instant at which the specimen was first filed from the lab interface system. |
| SPECIMEN_INACTIVE_UTC_DTTM | DATETIME (UTC) | The instant at which the specimen was rendered inactive in response to a laboratory information system (LIS) message or linked order status change. |
| SPECIMEN_INACTIVE_C | INTEGER |  |
